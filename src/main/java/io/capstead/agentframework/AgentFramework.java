...
public class AgentFramework implements Runnable{
 ...
 static class WorkItemShow extends DbCommand implements Callable<Integer>{
  ...
  public Integer call()throws Exception{try(var store=new SqliteKnowledgeStore(db)){var item=store.workItem(key);if(item.isEmpty()){System.err.println("Work item not found: "+key);return 2;}System.out.println(new ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(item.get()));}return 0;}
 }
